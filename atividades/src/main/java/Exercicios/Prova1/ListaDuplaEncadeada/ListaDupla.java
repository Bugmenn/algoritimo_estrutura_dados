package Exercicios.Prova1.ListaDuplaEncadeada;
...
public class ListaDupla<T> {
    ...
    public ListaDupla<T> clonar() {
        ...
        // pega os valores e adiciona em uma nova lista
        while (p != null) {
            novaLista.inserir(p.getInfo());
            p = p.getProximo();
        }

        ListaDupla<T> listaInvertida = new ListaDupla<>();
        NoListaDupla<T> q = novaLista.getPrimeiro();

        // inverte a lista
        while (q != null) {
            listaInvertida.inserir(q.getInfo());
            q = q.getProximo();
        }
